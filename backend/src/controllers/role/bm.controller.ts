...
export const bmDashboard = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (user.role !== RoleId.branchManager) return res.status(403).json({ message: "Forbidden: BM only" });

    const scopedBranchIds = user.branchScope || [];

    const [branches, approvals, visits, notifications] = await Promise.all([
      // Branches in scope — only KPI fields BM home uses
      ...
      // Approvals in scope — only fields BM approvals screen uses
      ...
      // Visits in scope — all fields (visits screen needs them)
...
export const bmAttendance = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const [attendanceLogs, staffUsers, scopedTasks] = await Promise.all([
      // Attendance logs for users in scoped branches — lean shape with weeklyTasks
      ...
      // Users in scope — only fields BM attendance screen uses
      ...
      // Today's tasks for scoped branches — for "Today's Queue" display
...
/**
 * GET /api/bm/tasks
 * Returns tasks for branches in the BM's scope — lean fields only.
 */
...
export const bmRejectComplaint = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const existing = await prisma.complaint.findUnique({
      where: { id },
      select: { id: true, branchId: true, status: true, timeline: true },
    });
    ...
    const updated = await prisma.complaint.update({
      where: { id },
      data: { status: ComplaintStatus.Rejected, timeline: timelineArr },
      select: { id: true, title: true, status: true, branchId: true, timeline: true, updatedAt: true },
    });
...
export const bmEditTask = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const existing = await prisma.task.findUnique({ where: { id }, select: { id: true, branchId: true, status: true } });
    ...
    return res.status(200).json({ task: { ...updated, assignedTo: updated.assignedToId, assignedBy: updated.assignedById } });
  } catch (error: any) {
  ...
  try {
    ...
    const updated = await prisma.visit.update({
      ...
      data: {
        ...
        report: `Cancelled: ${reason || "No reason provided"} — ${new Date().toISOString()}`,
      },
      select: { id: true, branchId: true, purpose: true, scheduledAt: true, status: true, agenda: true, report: true, managerId: true },
    });
