...
import { CheckCircle2, Clock, XCircle, Building, Users, Search, CalendarDays, ChevronLeft, ChevronRight, ListChecks, RefreshCw, ClipboardCheck } from "lucide-react-native";
...
export function BranchManagerAttendanceScreen() {
  ...
  const [taskTypeFilter, setTaskTypeFilter] = useState<"all" | "daily" | "weekly">("all");
          ...
          <Card variant="glass">
            ...
            <View style={{ flexDirection: "row", gap: spacing.sm, marginBottom: spacing.md }}>
              {(["all", "daily", "weekly"] as const).map((type) => (
                <TouchableOpacity
                  key={type}
                  onPress={() => setTaskTypeFilter(type)}
                  style={{ paddingHorizontal: spacing.lg, paddingVertical: spacing.sm, borderRadius: borderRadius.full, borderWidth: 1, backgroundColor: taskTypeFilter === type ? colors.brand : colors.white, borderColor: taskTypeFilter === type ? colors.brand : colors.border }}
                >
                  <Text style={{ fontSize: fontSize.xs, color: taskTypeFilter === type ? colors.white : colors.slate700 }}>{type === "all" ? "All Tasks" : type === "daily" ? "Daily" : "Weekly"}</Text>
                </TouchableOpacity>
              ))}
            </View>
              ...
              {filteredStaff.map((staff) => {
                ...
                const mergedTasks = [
                  ...
                  ...staffTasks.filter(st => !enteredTitles.has(st.title.toLowerCase().trim())).map(st => ({
                    ...
                    title: st.title,
                    status: st.status,
                    isWeekly: st.schedule === "Weekly" || st.applianceId != null
                  }))
                ];

                const filteredMergedTasks = taskTypeFilter === "all"
                  ? mergedTasks
                  : mergedTasks.filter(t => taskTypeFilter === "weekly" ? t.isWeekly : !t.isWeekly);

                const weeklyCount = mergedTasks.filter(t => t.isWeekly).length;
                const dailyCount = mergedTasks.filter(t => !t.isWeekly).length;
                    ...
                    {mergedTasks.length > 0 ? (
                      <View style={{ marginTop: spacing.lg, borderTopWidth: 1, borderColor: colors.slate100, paddingTop: spacing.md, gap: spacing.xs }}>
                        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: spacing.xs }}>
                          <Text style={{ fontSize: fontSize.xs, color: colors.slate400, textTransform: "uppercase", letterSpacing: 1 }}>
                            {taskTypeFilter === "weekly" ? "Weekly Tasks" : taskTypeFilter === "daily" ? "Daily Tasks" : "Today's Queue"}
                          </Text>
                          <View style={{ flexDirection: "row", gap: spacing.sm }}>
                            {dailyCount > 0 && <Text style={{ fontSize: fontSize.xs, color: colors.brandSecondary }}>{dailyCount} daily</Text>}
                            {weeklyCount > 0 && <Text style={{ fontSize: fontSize.xs, color: colors.info }}>{weeklyCount} weekly</Text>}
                          </View>
                        </View>
                        {filteredMergedTasks.length > 0 ? filteredMergedTasks.map(t => (
                          <View key={t.id} style={{ flexDirection: "row", alignItems: "center", gap: spacing.sm }}>
                            {t.isWeekly ? (
                              <RefreshCw size={14} color={t.status === "Completed" ? colors.success : colors.info} />
                            ) : (
                              <ListChecks size={14} color={t.status === "Completed" ? colors.success : colors.brandSecondary} />
                            )}
                            <Text style={{ fontSize: fontSize.sm, color: t.status === "Completed" ? colors.slate400 : colors.slate700, textDecorationLine: t.status === "Completed" ? "line-through" : "none" }}>
                              {t.title}
                            </Text>
                            {t.isWeekly && <Badge label="Weekly" type="Info" />}
                          </View>
                        )) : (
                          <Text style={{ fontSize: fontSize.sm, color: colors.slate400, fontStyle: "italic" }}>No {taskTypeFilter} tasks</Text>
                        )}
                      </View>
                    ) : (
