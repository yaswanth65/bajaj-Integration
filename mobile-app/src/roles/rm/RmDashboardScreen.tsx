            ...
            <View style={{ backgroundColor: colors.slate900, borderRadius: borderRadius["5xl"], padding: spacing["2xl"], borderWidth: 1, borderColor: "rgba(255,255,255,0.1)" }}>
              ...
              <View style={{ marginTop: spacing.xl, gap: spacing.md }}>
                {(() => {
                  ...
                  scopedComplaints.filter((c) => c.status === "Escalated" && activeBranchIds.includes(c.branchId)).forEach((c) => {
                    const b = scopedBranches.find((br) => br.id === c.branchId);
                    items.push(`${b?.name || "Branch"}: ${c.title} is escalated.`);
                  });
            ...
            <Card variant="glass">
              ...
              <View style={{ marginTop: spacing.xl, gap: spacing.md }}>
                {(() => {
                  ...
                  scopedComplaints.filter((c) => c.estimatedCost > 20000 && c.status !== "Resolved" && activeBranchIds.includes(c.branchId)).forEach((c) => {
                    const b = scopedBranches.find((br) => br.id === c.branchId);
                    items.push({ text: `Approve ${c.estimatedCost > 40000 ? "capex" : "repair"} for ${b?.name || "branch"} — ${c.title}.`, priority: c.priority });
                  });
