...
export function RmMonitoringScreen() {
  ...
  const activeTab = state.tabs.managerMonitoring === "lc" || state.tabs.managerMonitoring === "all" ? "daily" : state.tabs.managerMonitoring || "daily";
  ...
  const filteredTasks = useMemo(() => {
    ...
    return scopedTasks.filter((t) => {
      ...
      if (activeTab === "daily" && t.schedule !== "Daily") return false;
      if (activeTab === "weekly" && t.schedule !== "Weekly" && t.schedule !== "One-Time" && t.applianceId == null) return false;
  ...
  return (
    <ScreenWrapper>
      ...
      {/* STEP 3: CONTROLS & TASK LIST (revealed only after branch selected) */}
      {selectedRegion !== "" && selectedBranchId !== "" ? (
        <>
          <View style={{ marginTop: spacing.xl }}>
            <SegmentedControl
              tabs={[
                { label: "Daily Tasks", value: "daily" },
                { label: "Weekly Tasks", value: "weekly" },
          ...
          <View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.md, marginTop: spacing.xl }}>
            <View style={{ flex: 1, minWidth: 200, flexDirection: "row", alignItems: "center", backgroundColor: colors.white, borderRadius: borderRadius.lg, paddingHorizontal: spacing.md, borderWidth: 1, borderColor: colors.border }}>
              ...
              <TextInput
                ...
                placeholder={activeTab === "attendance" ? "Search staff..." : "Search tasks..."}
                      ...
                      {filteredTasks.length > visibleTasksLimit && (
                        ...
                        >
                          ...
                          <Text style={{ fontSize: fontSize.sm, fontWeight: "600", color: colors.slate600 }}>
                            Load More (+{filteredTasks.length - visibleTasksLimit} tasks remaining)
                          </Text>
                        </TouchableOpacity>
                      )}
                    </>
        ...
        selectedRegion !== "" && (
          <Card style={{ marginTop: spacing.xl, padding: spacing.xl, borderStyle: "dashed", borderWidth: 1, borderColor: colors.border, alignItems: "center" }}>
            <Layers size={24} color={colors.slate300} style={{ marginBottom: spacing.sm }} />
            <Text style={{ fontSize: fontSize.sm, color: colors.slate400 }}>Select a branch or "All Branches" to load {activeTab === "attendance" ? "attendance" : "tasks"}</Text>
          </Card>
        )
      )}
    </ScreenWrapper>
