...
export function FormModal({ visible, onClose }: Props) {
  ...
  const resetForms = () => {
    setTaskTitle(""); setTaskAudience("lc"); setTaskBranch(String(branches[0]?.id || "")); setTaskSchedule("Daily"); setTaskZone(""); setTaskDeadline(""); setTaskPriority("High"); setTaskProofRule("true"); setTaskNotes("");
  ...
  const forms: Record<string, { title: string; subtitle: string; render: () => React.ReactNode }> = {
    ...
    complaint: {
      ...
      render: () => {
        const handleSubmit = () => {
          const result = validate(complaintSchema, { title, type, priority, description: desc });
          if (!result.success) { setErrors(result.errors); return; }
          createComplaint({ title, type, priority: priority as any, description: desc });
          handleClose();
        };
        return (
          <View style={{ gap: spacing.lg }}>
            {input("Issue title", title, setTitle, undefined, "title")}
            {selectOptions("Type", ["Appliance", "Electrical", "Plumbing", "Safety", "Security"], type, setType, "type")}
            {selectOptions("Priority", ["Low", "Medium", "High", "Critical"], priority, setPriority, "priority")}
            {textarea("Describe issue, exact area, visible damage, workaround and impact", desc, setDesc, "description")}
    ...
    task: {
      title: "Task form",
      subtitle: "Create and assign tasks to any role with proof requirements.",
      render: () => {
        const handleSubmit = () => {
          ...
          if (!result.success) { setErrors(result.errors); return; }
          createTask({ title: taskTitle, audience: taskAudience as RoleId, schedule: taskSchedule as any, zone: taskZone, deadline: taskDeadline, priority: taskPriority as any, proofRequired: taskProofRule === "true", notes: taskNotes, branchId: taskBranch });
          handleClose();
        };
        ...
        return (
          <View style={{ gap: spacing.lg }}>
            ...
            {submitBtn("Create task", handleSubmit)}
          </View>
        );
      },
    },
